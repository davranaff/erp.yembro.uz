...
class SlaughterProcessing(Base, IDMixin, TimestampMixin):
    ...
    birds_processed: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    first_sort_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    second_sort_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    bad_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="slaughter_processings"
    )
    department: Mapped["Department"] = relationship(
        "Department", back_populates="slaughter_processings"
    )
    arrival: Mapped["SlaughterArrival"] = relationship(
        "SlaughterArrival", back_populates="processings", lazy="selectin"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "birds_processed >= 0",
            name="ck_slaughter_processing_birds_processed_non_negative",
        ),
