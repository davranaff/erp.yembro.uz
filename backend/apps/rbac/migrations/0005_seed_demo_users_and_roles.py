...
ROLE_DEFINITIONS = [
    ...
    },
    {
        ...
        "permissions": {
            # ledger=admin → фронт считает бухгалтера за org-admin для
            # cashbox-view, поэтому видны ВСЕ кассы (включая «общие» с
            # module=NULL).
            "ledger": "admin",
            "reports": "admin",
            "sales": "admin",
            ...
            "feedlot": "r",
            "slaughter": "r",
            "admin": "r",          # просмотр аудита/ролей
            # Намеренно БЕЗ core, feed, vet — бухгалтер не должен видеть
            # в сайдбаре группы «Ядро» (Номенклатура/Блоки) и «Обеспечение»
            # (Корма/Профили усушки/Вет.аптека/Токены продавцов).
        },
    },
]
