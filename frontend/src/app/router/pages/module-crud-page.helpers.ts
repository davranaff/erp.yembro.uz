...
const resourceUiConfigs: Record<string, ResourceUiConfig> = {
  ...
  'feed:ingredients': {
    formOrder: ['name', 'code', 'unit', 'description', 'is_active'],
    tableOrder: ['name', 'code', 'unit', 'is_active'],
    hiddenFields: ['measurement_unit_id'],
    hideOrganizationFieldWhenScoped: true,
  },
  'feed:formulas': {
    formOrder: ['name', 'code', 'feed_type_id', 'description', 'is_active'],
    tableOrder: ['name', 'code', 'feed_type_id', 'is_active'],
    hideOrganizationFieldWhenScoped: true,
  },
  'feed:consumptions': {
    formOrder: [
  ...
  'feed:shrinkage-profiles': {
    ...
    fieldHelpers: {
      target_type: {
        ru: 'Что сохнет: сырьё (ингредиент) или готовый корм (тип). От выбора зависит, какое поле ниже будет активно.',
        uz: 'Nima quriydi: xomashyo (ingredient) yoki tayyor em (turi). Shunga qarab pastdagi maydon faollashadi.',
        en: 'What shrinks: a raw ingredient or a finished feed type. The choice drives which field below is required.',
      },
      ingredient_id: {
        ru: 'Заполните, только если выбрали «Сырьё». Профиль применится ко всем приходам этого ингредиента.',
        uz: 'Faqat «Xomashyo» tanlangan bo‘lsa to‘ldiring. Profil shu ingredientning barcha kelishlariga qo‘llanadi.',
        en: 'Fill only when target = raw ingredient. The profile applies to every arrival of this ingredient.',
      },
      feed_type_id: {
        ru: 'Заполните, только если выбрали «Готовый корм». Профиль применится ко всем произведённым партиям этого типа.',
        uz: 'Faqat «Tayyor em» tanlangan bo‘lsa to‘ldiring. Profil shu turdagi barcha ishlab chiqarilgan partiyalarga qo‘llanadi.',
        en: 'Fill only when target = finished feed. The profile applies to every production batch of this feed type.',
      },
      warehouse_id: {
        ru: 'Склад, где действует правило. Оставьте пустым — правило будет общее по организации; склад-специфичное правило имеет приоритет над общим.',
        uz: 'Qoida qo‘llanadigan ombor. Bo‘sh qoldiring — qoida tashkilot bo‘yicha umumiy bo‘ladi; omborga bog‘langan qoida umumiysidan ustun.',
        en: 'Warehouse where the rule applies. Leave empty for an org-wide default; warehouse-specific rules win over the default.',
      },
      period_days: {
        ru: 'Каждые сколько дней списывается процент. Пример: 7 — раз в неделю.',
        uz: 'Har necha kunda foiz chiqariladi. Masalan: 7 — haftasiga bir marta.',
        en: 'How often the percent is applied, in days. Example: 7 means weekly.',
      },
      percent_per_period: {
        ru: 'Сколько процентов от ТЕКУЩЕГО остатка списывается за один период. Компаундно: после первого списания остаток сохнет медленнее.',
        uz: 'Bir davrda JORIY qoldiqdan necha foiz chiqariladi. Kompaund: birinchi chiqimdan so‘ng qoldiq sekinroq quriydi.',
        en: 'Percent of the CURRENT remaining lot weight taken each period. Compound: after the first step the lot dries more slowly.',
      },
      max_total_percent: {
        ru: 'Верхний предел накопленной усушки по партии, %. Когда лимит достигнут — списания останавливаются. Оставьте пустым, чтобы предела не было.',
        uz: 'Partiya bo‘yicha umumiy qurishning yuqori chegarasi, %. Chegaraga yetsa chiqimlar to‘xtaydi. Chegara kerak bo‘lmasa bo‘sh qoldiring.',
        en: 'Cap on the total accumulated shrinkage per lot, %. When reached, shrinkage stops. Leave empty for no cap.',
      },
      stop_after_days: {
        ru: 'Прекратить усушку через N дней от поступления партии, даже если лимит не достигнут. Оставьте пустым — усушка продолжается бессрочно.',
        uz: 'Partiya kelganidan N kun o‘tgach qurishni to‘xtatish, chegaraga yetmagan bo‘lsa ham. Bo‘sh qoldiring — qurish cheksiz davom etadi.',
        en: 'Stop shrinkage N days after the lot arrived, even if the cap is not yet hit. Leave empty for no time limit.',
      },
      starts_after_days: {
        ru: 'Грейс-период: первые N дней партия ещё не усыхает. По умолчанию 0 — начинается с первого дня.',
        uz: 'Grace davri: birinchi N kun partiya quriy boshlamaydi. Standart 0 — birinchi kundan boshlanadi.',
        en: 'Grace period: the lot does not shrink during the first N days. Default is 0 — starts from day one.',
      },
    },
    hideDepartmentFieldWhenScoped: true,
    hideOrganizationFieldWhenScoped: true,
  },
  'feed:shrinkage-state': {
    formOrder: [
      'lot_type',
      'lot_id',
      'profile_id',
    ...
    fieldHelpers: {
      is_frozen: {
        ru: 'Если включено — ночной воркер больше не трогает эту партию. Используйте, чтобы вручную «запереть» усушку после возврата/списания.',
        uz: 'Yoqilgan bo‘lsa — tungi vorker bu partiyaga tegmaydi. Qaytarish/hisobdan chiqarishdan so‘ng qurishni qo‘lda «qulflash» uchun ishlating.',
        en: 'When on, the nightly worker stops touching this lot. Use to manually freeze shrinkage after a return or write-off.',
      },
    },
    hideDepartmentFieldWhenScoped: true,
    hideOrganizationFieldWhenScoped: true,
