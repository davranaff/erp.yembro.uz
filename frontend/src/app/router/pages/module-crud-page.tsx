...
const getDefaultWarehouseId = (warehouses: WarehouseRecord[]): string => {
  ...
  return defaultWarehouse ? getWarehouseId(defaultWarehouse) : '';
};
...
export function ModuleCrudPage() {
  ...
  const rawResources = moduleConfig?.resources ?? [];
  ...
  const pageSize = 12;
  const translateReferenceLabel = (
    field: CrudFieldMeta,
    optionValue: string,
    optionLabel: string,
  ): string => getReferenceOptionLabel(field, optionValue, optionLabel, t);
  ...
  const actorProfileQuery = useApiQuery({
    ...
    enabled:
      Boolean(authSession?.employeeId) &&
      (!authSession?.organizationId || authSession?.departmentId == null),
  });
  ...
  const departmentQuery = useApiQuery<CrudListResponse>({
    ...
    enabled: Boolean(moduleConfig?.isDepartmentAssignable && authSession?.employeeId),
  });
  ...
  const departmentModuleKey = moduleConfig?.isDepartmentAssignable ? moduleConfig.key : null;
  const availableDepartments = useMemo(() => {
    if (!moduleConfig?.isDepartmentAssignable) {
  ...
  }, [allDepartments, departmentModuleKey, moduleConfig, moduleKey]);
  ...
  const activeQuickRangeKey = (() => {
    const matchingPresetKeys = quickRangePresets
      .filter((preset) => preset.startDate === effectiveStartDate && preset.endDate === effectiveEndDate)
      .map((preset) => preset.key);

    if (validQuickRangeKeys.has(requestedPeriodKey) && matchingPresetKeys.includes(requestedPeriodKey)) {
  ...
  const activeMedicineBatchQrSheetPayload =
    medicineBatchQrSheetBatch?.id ? medicineBatchQrCache[medicineBatchQrSheetBatch.id] : undefined;
  ...
  const inventoryMovementKind =
    typeof formValues.movement_kind === 'string' ? formValues.movement_kind.trim().toLowerCase() : '';
  ...
  const warehouseById = useMemo(
    () => new Map(availableWarehouses.map((warehouse) => [getWarehouseId(warehouse), warehouse] as const)),
    [availableWarehouses],
  );
  ...
  const movementCounterpartyWarehouseRecords = useMemo(() => {
    ...
    return availableWarehouses.filter((warehouse) => {
      const warehouseId = getWarehouseId(warehouse);
      return isWarehouseActive(warehouse) && warehouseId.length > 0 && warehouseId !== selectedWarehouseId;
    });
  }, [
    availableWarehouses,
  ...
  const movementWarehouseIds = useMemo(
    () => new Set(movementWarehouseRecords.map((warehouse) => getWarehouseId(warehouse)).filter(Boolean)),
  ...
  const clientNotificationTemplates = useMemo(
    () =>
      isClientNotificationBulkMode
        ...
        : (clientNotificationContextQuery.data?.templates ?? CLIENT_NOTIFICATION_FALLBACK_TEMPLATES),
    [clientNotificationContextQuery.data?.templates, isClientNotificationBulkMode],
  );
  const activeClientNotificationTemplate = useMemo(
    () =>
      clientNotificationTemplates.find((template) => template.key === clientNotificationTemplateKey) ??
  ...
  const hiddenFieldNames = useMemo(() => {
    ...
    if (!canManageHrSecurityFields && resourceModuleKey === 'hr' && activeResource?.key === 'employees') {
      nextHiddenFields.add('role_ids');
    }

    if (!canManageHrSecurityFields && resourceModuleKey === 'hr' && activeResource?.key === 'roles') {
  ...
  const unresolvedManagedFields = useMemo(() => {
    if (selectedRecordId) {
      return [] as string[];
    }

    const nextFields: string[] = [];
    const isDepartmentFieldManaged = formHiddenFieldNames.has('department_id');
    const isOrganizationFieldManaged = formHiddenFieldNames.has('organization_id');
    const isCreatorFieldManaged =
      formHiddenFieldNames.has('created_by') || formHiddenFieldNames.has('created_by_id');

    if (
      editableFields.some((field) => field.name === 'department_id') &&
      isDepartmentFieldManaged &&
      !fallbackDepartmentId
    ) {
      nextFields.push('department_id');
    }

    if (
      editableFields.some((field) => field.name === 'organization_id') &&
      isOrganizationFieldManaged &&
      !fallbackOrganizationId
    ) {
      nextFields.push('organization_id');
    }

    if (isCreatorFieldManaged && !actorEmployeeId) {
      nextFields.push('created_by');
    }
    ...
    actorEmployeeId,
    editableFields,
    formHiddenFieldNames,
    fallbackDepartmentId,
    fallbackOrganizationId,
    selectedRecordId,
  ]);
  const isSystemContextMissing = unresolvedManagedFields.length > 0;
  ...
  const getRecordSummaryLabel = (record: CrudRecord): string => {
  ...
  useEffect(() => {
    ...
    setAuditRecordLabel('');
    setIsAuditSheetOpen(false);
  }, [auditRecordId, idColumn, visibleRecords]);
  ...
  >({
    ...
    onSuccess: (result) => {
      ...
      setClientNotificationFeedback(
        result.error ||
          t(
            'crud.clientNotificationSendFailed',
            undefined,
  ...
  const resetClientNotificationState = () => {
    setClientNotificationTargetId('');
    setClientNotificationTargetLabel('');
    setClientNotificationTemplateKey('debt_reminder');
    setClientNotificationMessage('');
  ...
  const handleOpenBulkClientNotification = () => {
    ...
    if (visibleClientIds.length === 0) {
      setOperationMessage(
        t(
          'crud.clientNotificationNoClients',
          undefined,
          'Нет клиентов для массовой рассылки.',
        ),
      );
      return;
    }
  ...
  const handleSendClientNotification = () => {
    ...
    if (!clientNotificationTargetId) {
      setClientNotificationFeedback(
        t(
          'crud.clientNotificationNoTarget',
          undefined,
          'Клиент не выбран.',
        ),
      );
  ...
  useEffect(() => {
    ...
    if (!activeClientNotificationTemplate?.message) {
      return;
    }

    setClientNotificationMessage(activeClientNotificationTemplate.message);
  }, [
  ...
  }, [isClientNotificationSheetOpen, isClientResource]);
  ...
  const systemContextHint =
    !selectedRecordId && isSystemContextMissing
      ? t(
          'crud.systemFieldContextMissing',
          {
            fields: unresolvedManagedFields
              .map((fieldName) => t(`fields.${fieldName}`, undefined, humanizeKey(fieldName)))
              .join(', '),
          },
          `Для создания записи сначала определите служебные поля: ${unresolvedManagedFields
            .map((fieldName) => t(`fields.${fieldName}`, undefined, humanizeKey(fieldName)))
            .join(', ')}.`,
        )
      : '';
...
<html lang="en">
  ...
  const resolveMedicineBatchQr = async (
    ...
    if (!forceGenerate) {
      const cached = medicineBatchQrCache[batchId];
      if (cached) {
        return cached;
      }
    }
  ...
  const handleSubmit = (mode: SubmitMode) => {
    if (
      (mode === 'create' &&
        !canExecuteModuleCrudAction('create_record', moduleCrudActionAccess)) ||
      (mode === 'update' &&
        !canExecuteModuleCrudAction('update_record', moduleCrudActionAccess))
    ) {
      return;
    }
  ...
  const renderPaginationBar = () => {
    ...
    return (
      <div
        className={`${frostedPanelClassName} flex flex-col gap-3 px-4 py-4 xl:flex-row xl:items-center xl:justify-between`}
      >
          ...
          <tbody>
            {paginatedRecords.map((record, rowIndex) => {
              ...
              const isDeleteConfirming =
                recordId !== EMPTY_TEXT && deleteConfirmRecordId === recordId;
  ...
  const renderStockMovementCards = () => {
    ...
    return (
      <div className={`${frostedPanelClassName} overflow-hidden`} data-tour="inventory-movements-list">
        <div className="max-h-[680px] overflow-auto">
          <table className="min-w-full border-collapse text-left text-sm">
            <thead className="sticky top-0 z-10 bg-card">
              <tr className="border-primary/16 border-b">
            ...
            <tbody>
              {paginatedRecords.map((record) => {
                ...
                return (
                  <tr key={recordId || JSON.stringify(record)} className="border-primary/10 border-b last:border-b-0">
  ...
  return (
    <div className="space-y-6" data-tour="module-page">
      <input
        ...
        type="file"
        className="hidden"
        onChange={handleMedicineBatchAttachmentInputChange}
      />
                          ...
                          <Button
                            ...
                            data-tour={
                              resource.apiModuleKey === 'core' && resource.path === 'clients'
                                ...
                                : resource.apiModuleKey === 'core' && resource.path === 'client-debts'
        ...
        <div className="space-y-6">
          ...
          {!moduleStatsQuery.isLoading && !moduleStatsQuery.error && moduleStatsSection ? (
            <AnalyticsDashboardView
              ...
              minimalMode={useMinimalModuleUi}
            />
          ) : null}
        </div>
      ) : (
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
              <Card className={heroCardClassName}>
                <CardHeader className="space-y-3">
                  ...
                  {!useMinimalModuleUi ? (
                    <CardDescription className="leading-6 text-muted-foreground">
                      {t(
                        'crud.departmentAnalyticsDescription',
                        undefined,
                        'Для головного департамента доступен общий обзор по всему поддереву и общие данные по сотрудникам и финансам.',
                      )}
                    </CardDescription>
                  ) : null}
                </CardHeader>
                <CardContent className="space-y-4">
                  {!useMinimalModuleUi ? (
                    <div
                      className={`${frostedPanelClassName} px-4 py-4 text-sm text-muted-foreground`}
                    >
                      {t(
                        'crud.departmentAnalyticsHint',
                        undefined,
                        'Операционные записи ведутся в дочерних подразделениях. На этом уровне остаются аналитика, общие сотрудники и общие финансы.',
                      )}
                    </div>
                  ) : null}
                  {canOpenModuleAnalytics ? (
              ...
              <Card className={heroCardClassName} data-tour="module-subdepartments-card">
                <CardHeader className="space-y-3">
                  ...
                  {!useMinimalModuleUi ? (
                    <CardDescription className="leading-6 text-muted-foreground">
                      {t(
                        'crud.subdepartmentsDescription',
                        undefined,
                        'Выберите дочернее подразделение для полной работы с его данными и операциями.',
                      )}
                    </CardDescription>
                  ) : null}
                </CardHeader>
            ...
            <>
                ...
                <CardHeader className="relative gap-5 border-b border-border/60 pb-5 xl:flex-row xl:items-start xl:justify-between">
                  <div className="space-y-3">
                    <div className="space-y-2">
                      ...
                      {!useMinimalModuleUi ? (
                        <CardDescription className="max-w-3xl leading-6 text-muted-foreground">
                          {isInventoryMovementsResource || isWarehouseResource
                            ? currentDepartmentLabel || recordsScopeLabel
                            : isHeadDepartmentView
                              ? t(
                                  'crud.headDepartmentListDescription',
                                  undefined,
                                  'Общие данные выбранного головного департамента и всех его поддепартаментов.',
                                )
                              : currentDepartmentLabel
                                ? `${t('crud.listDescription')} ${t('crud.departmentFilter')}: ${currentDepartmentLabel}.`
                                : t('crud.listDescription')}
                        </CardDescription>
                      ) : null}
                    </div>
                ...
                >
                  <SheetHeader>
                    ...
                    {!useMinimalModuleUi ? (
                      <SheetDescription>
                        {t(
                          'crud.medicineBatchQrCenterDescription',
                          undefined,
                          'Сканируйте QR или откройте публичную карточку товара.',
                        )}
                      </SheetDescription>
                    ) : null}
                  </SheetHeader>
                      ...
                      <>
                        <div className="rounded-2xl border border-border/75 bg-card px-4 py-4 text-center shadow-[0_18px_46px_-34px_rgba(15,23,42,0.16)]">
                          <img
                            ...
                            alt="QR code"
                            className="mx-auto h-64 w-64 rounded-2xl border border-border/70 bg-white p-3 object-contain"
                          />
                        </div>
                        ...
                        <div className={`${frostedPanelClassName} space-y-2 px-4 py-4 text-xs`}>
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className="text-right text-foreground">
                              {formatDateTimeDisplayValue(activeMedicineBatchQrSheetPayload.generated_at)}
                            </span>
                          </div>
                          <div className="flex items-start justify-between gap-3">
                            <span className="text-muted-foreground">
                ...
                >
                  <SheetHeader>
                    ...
                    {!useMinimalModuleUi ? (
                      <SheetDescription>
                        {isClientNotificationBulkMode
                          ? t(
                              'crud.bulkClientNotificationDescription',
                              undefined,
                              'Для каждого клиента будет отправлен персонализированный текст на основе выбранного шаблона.',
                            )
                          : t(
                              'crud.clientNotificationDescription',
                              undefined,
                              'Выберите шаблон, при необходимости отредактируйте сообщение и отправьте в Telegram.',
                            )}
                      </SheetDescription>
                    ) : null}
                  </SheetHeader>
                  ...
                  <div className="space-y-4 p-4">
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {clientNotificationTemplates.map((template) => (
                          <Button
                            key={template.key}
                            type="button"
                            variant={template.key === clientNotificationTemplateKey ? 'default' : 'outline'}
                            className="rounded-full"
                            onClick={() => {
                      ...
                      {!useMinimalModuleUi && activeClientNotificationTemplate?.description ? (
                        <p className="text-xs text-muted-foreground">
                          {activeClientNotificationTemplate.description}
                        </p>
                      ) : null}
                    </div>
                    ...
                    >
                      <label className="text-sm font-medium text-foreground" htmlFor="client-notification-message">
              ...
              >
                <CrudDrawer
                  ...
                  description={
                    useMinimalModuleUi || isInventoryMovementsResource || isWarehouseResource
                      ? ''
                      : t('crud.formDescription')
                  }
                  formProps={{
                    onSubmit: (event) => {
                      event.preventDefault();
                ...
                >
                  ...
                  {!useMinimalModuleUi && !systemFormError && systemContextHint ? (
                    <div className="rounded-2xl border border-border/75 bg-card px-4 py-3 text-sm text-muted-foreground shadow-[0_16px_36px_-30px_rgba(15,23,42,0.1)]">
                      {systemContextHint}
                    </div>
                  ) : null}
              ...
              >
                <CrudDrawer
                  ...
                  description={useMinimalModuleUi ? '' : t('crud.auditDescription')}
                  footer={
                    <CrudDrawerFooter
                      closeLabel={t('common.close')}
                      onClose={() => {
                ...
                >
                  ...
                  <div className="space-y-4" data-tour="module-audit-history">
                    ...
                    {auditHistoryQuery.error ? <ErrorNotice error={auditHistoryQuery.error} /> : null}
