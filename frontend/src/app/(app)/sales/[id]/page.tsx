...
import { useSaleOrderSummary } from '@/hooks/useSales';
...
export default function SaleDetailPage() {
  ...
  const { data, isLoading, error } = useSaleOrderSummary(id);
  ...
  return (
    <>
      <div className="page-hdr">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <button className="btn btn-ghost btn-sm" onClick={() => router.push('/sales')}>
          ...
          <div className="sub" style={{ marginTop: 4 }}>
            ...
            {o.customer_id ? (
              <Link
                href={`/counterparties/${o.customer_id}`}
      ...
      {/* Один большой блок: Долг по счёту */}
      <div style={{
        marginTop: 4,
        padding: '20px 24px',
        borderRadius: 8,
        ...
        <div style={{ fontSize: 12, color: 'var(--fg-2)', textTransform: 'uppercase', letterSpacing: 0.5 }}>
          {outstanding > 0 ? 'Не оплачено' : 'Полностью оплачено'}
        </div>
        <div style={{
        ...
        <div style={{ fontSize: 13, color: 'var(--fg-2)', marginTop: 8 }}>
          Сумма счёта: <b>{fmt(o.amount_uzs)}</b> сум · Уже оплачено: <b>{fmt(o.paid_amount_uzs)}</b> сум
