 ...
 * транзферов нет (или query 403/empty) — панель не рендерится.
 *
 * Кнопка «Принять» открывает `AcceptTransferModal` где оператор
 * выбирает склад приёмки и (опционально) блок. Backend требует
 * `to_warehouse_id` обязательно — иначе fallback на склад отправителя
 * сделает учёт невалидным.
 */
export default function IncomingTransfersPanel({
  ...
  const accepting = acceptingId
    ? transfers.find((t) => t.id === acceptingId) ?? null
    : null;
